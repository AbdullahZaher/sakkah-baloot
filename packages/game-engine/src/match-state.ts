...
export function createMatchState(
  ...
  return {
    matchId,
    stateVersion: 0,
    roundId: createRoundId(matchId, roundNumber),
    roundNumber,
    dealerSeat,
    phase: "ROUND_ACTIVE",
    score,
    lastRoundScore: null,
    end: { status: "ONGOING", score },
    round,
  };
}

export function completeMatchRound(
  state: MatchState,
  roundScore: RoundScoreBreakdown,
  round?: RoundState,
): MatchState {
  if (state.phase !== "ROUND_ACTIVE") {
    throw new Error("Only an active round can be completed");
  }
  ...
  return {
    ...
    phase: end.status === "FINISHED" ? "MATCH_COMPLETE" : "ROUND_COMPLETE",
    score,
...
export function startNextRound(
  ...
  if (state.phase !== "ROUND_COMPLETE") {
    throw new Error("Next round requires a completed non-final round");
  }
  ...
  return {
    ...state,
    stateVersion: state.stateVersion + 1,
    roundId: createRoundId(state.matchId, roundNumber),
    roundNumber,
    dealerSeat,
    phase: "ROUND_ACTIVE",
    lastRoundScore: null,
    end: { status: "ONGOING", score: state.score },
    round: round ?? null,
  };
}
